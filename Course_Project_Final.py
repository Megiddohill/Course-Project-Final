...
def Login():
    ...
    user_role = "None"
    while True: 
        userDetail = userFile.readline() # remember this just reads a line at a time
        if not userDetail:
            return user_role, user_name
        userDetail = userDetail.replace("\n", "")
        userList = userDetail.split("|")
        if user_name == userList[0]:
            user_role = userList[2] #if the username is valid, it will return [2] which is the "role"
            return user_role, user_name
    return user_role, user_name
...
if __name__ == "__main__":
    user_role, user_name = Login() # from our new function
    empl_totals = {}
    DetailsPrint = True
    if (user_role.upper() == "NONE"):
